...
// 23.06.2019 добавлен разбор искаженной конструкции без запятой
//            "Действует скидка которая вас порадует"
...
wordset ПредлогДоПосле = { предлог:до{}, предлог:после{} }
pattern ГруппаНареч1
{
 a=НачНаречнойГруппы_ДоПосле:export { ТИП_МОДИФ }
 prepos=ПредлогДоПосле{node:root_node}
 n=СущСРодДоп{ Падеж:РОД }
} : links { prepos.{<ATTRIBUTE>a <OBJECT>n} }
  : ngrams { 1 }
*/
...
// Искаженная конструкция - нет запятой:
// Действует скидка которая вас порадует
pattern СущСПричОборотом
{
 n=ОсноваДляПрисоедКоторый :export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
