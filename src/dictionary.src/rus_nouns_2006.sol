...
automat sg
{
 ...
 // Для имен, для которых есть употребимая форма мн.ч.
 #define имя2(_СущИм) \
  #begin
  entry _СущИм : Существительное { paradigm Сущ_2006 ПЕРЕЧИСЛИМОСТЬ:ДА CharCasing:FirstCapitalized flag flexer { "pl" } }
  link СУЩЕСТВИТЕЛЬНОЕ:_СущИм{ ОДУШ:ОДУШ } <в_класс> СУЩЕСТВИТЕЛЬНОЕ:ИМЯ {}
  #end
 ...
 имя( Сюзанна )
 имя( Тамара )
 имя2( Татьяна ) // Волочкова показала своего жениха Татьянам.
 имя( Тереза )
 имя( Улита )
 имя( Ульяна )
 имя( Устина )
