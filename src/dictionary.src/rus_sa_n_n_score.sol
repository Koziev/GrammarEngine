...
// Ему даже делают специальный массаж лица.
fact сущ_сущ
{
 if context { существительное:массаж{} существительное:лицо{ падеж:род одуш:неодуш } }
  then return 5
}
...
СущСущ(душа, смертный, 5) // Я создала тела и души смертных
СущСущ(удар, нога, 5) // ударом ноги выбили дверь .
СущСущ(польза, дело, 5) // Для пользы дела.
СущСущ(поле, рожь, 5) // Началось поле ржи.
СущСущ(правило, пользование, 5) // Правила пользования велосипедами.
СущСущ(поле, битва, 5) // И полем битвы.
СущСущ(поиск, цель, 5) // Начали поиск цели.
СущСущ(пожатие, рука, 5) // Попрощались пожатием руки. 
СущСущ(правило, поведение, 6) // Правила поведения на воде
СущСущ(план, обезвреживание, 2) // Разработали план обезвреживания.
СущСущ(кусок, пирог, 5) // Кусок пирога сунула.
СущСущ(наводнение, веко, -10) // Англия переживает наводнение века
СущСущ(наводнение, век, 1)
СущСущ(отбой, тревога, 5) // Дали отбой тревоги.
СущСущ(опушка, лес, 5) // Шли опушкой леса.
СущСущ(обрыв, крепление, 5) // Заметили обрыв крепления?
СущСущ(рассмотрение, дело, 5) // Законность при рассмотрении дел арбитражным судом
СущСущ(дно, воронка, 5) // На дне воронки лежит чернильница.
СущСущ(правило, техника, 5) // некоторые сотрудники, как выяснилось в ходе расследования, не соблюдали элементарных правил техники безопасности
СущСущ(представитель, оппозиция, 5) // Участвовала в распространении видеороликов, компрометирующих независимых журналистов и представителей оппозиции
СущСущ(устранение, симптом, 5) // они могут использоваться для лечения тревожных состояний или для кратковременного устранения симптомов тревоги
СущСущ(развязка, событие, 5) // Решив, что искать его здесь никто не будет, он распластался на камне и стал ждать развязки событий.
СущСущ(урегулирование, конфликт, 5) // находимся на стадии серьезных переговоров по урегулированию нагорно-карабахского конфликта
СущСущ(выполнение, миссия, 5) // После выполнения финальной миссии можно найти проход в секретную зону последнего уровня.
СущСущ(президент, республика, 5) // Кандидатуру будущего министра обязательно согласуют с действующим президентом республики
СущСущ(достижение, цель, 5) // Желание совместной работы с другими организациями и людьми для достижения общих целей
СущСущ(район, Казань, 5) // Андрей Белов работал в должности главы администрации Авиастроительного района Казани.
СущСущ(последствие, падение, 5) // Игра заключалась в моделировании последствий серьезного падения мировой добычи нефти
СущСущ(снос, дом, 5) // Предполагается, что решение о сносе дома останется за самими жителями города.
СущСущ(место, жительство, 5) // Способы прописки новорожденного по месту жительства.
СущСущ(начальник, станция, 5) // Разыскали начальника станции.
СущСущ(право, инициатива, 5) // Курских прокуроров наделили правом нормотворческой инициативы
СущСущ(выделение, моча, 10) // После мочеиспускания постоянное выделение мочи по каплям.
СущСущ(недержание, моча, 10) // Недержание мочи у детей из-за слабости сфинктеров.
СущСущ(болезнь, век, 5) // Аллергию многие врачи считают болезнью века.
СущСущ(режим, госграница, 5) // Ему ранее инкриминировали нарушение режима госграницы.
СущСущ(растаможка, техника, 2) // Ей не доверили растаможку бытовой техники.
СущСущ(глоток, воздух, 5) // Ей наконец удалось вдохнуть глоток воздуха.
...
СущСущ(луч, обожание, 5) // Изливают любвеобильные лучи обожания
СущСущ(поле, брань, 5) // Ему не везло на поле брани;
// -------------------------------------------------------
...
СущДат(памятник, стрелок, 4) // памятник красным стрелкам
