...
StorageConnection_SQLITE::~StorageConnection_SQLITE()
{
    if (hdb != nullptr)
    {
        sqlite3_close(hdb);
...
TransactionGuard* StorageConnection_SQLITE::GetTx()
{
    return new TransactionGuard_SQLITE(hdb);
}

TransactionGuard* StorageConnection_SQLITE::GetReadTx()
{
    return new ReadTransactionGuard_SQLITE(hdb);
}


void StorageConnection_SQLITE::BeginTx()
{
    lem::sqlite_begin_tx(hdb);
}


void StorageConnection_SQLITE::CommitTx()
{
...
void StorageConnection_SQLITE::RollbackTx()
{
...
void StorageConnection_SQLITE::BeginReadTx()
{
    lem::sqlite_begin_tx(hdb);
}


void StorageConnection_SQLITE::EndReadTx()
{
    lem::sqlite_commit_tx(hdb);
    return;
}
...
void StorageConnection_SQLITE::Error()
{
    lem::MemFormatter mem;
...
int StorageConnection_SQLITE::GetLastId()
{
    return (int)sqlite3_last_insert_rowid(hdb);
}

bool StorageConnection_SQLITE::IsConnected() const
{
    return hdb != nullptr;
}
