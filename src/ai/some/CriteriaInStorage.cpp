...
CriteriaInStorage::CriteriaInStorage(void)
{}
...
const Criterion& CriteriaInStorage::operator[]( int id )
{
 ...
 std::map<int,const Criterion*>::const_iterator it = id2x.find(id);
 if( it!=id2x.end() )
  return *it->second;
...
CriterionEnumerator* CriteriaInStorage::Enumerate(void)
{
