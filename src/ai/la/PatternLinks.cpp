...
PatternLinks::PatternLinks(void)
{
}
...
{
 for( lem::Container::size_type i=0; i<groups.size(); ++i )
  groups[i]->Generate( la, PatternSequenceNumber, cur_result );

 for( lem::Container::size_type i=0; i<links.size(); ++i )
  links[i]->Generate( la, PatternSequenceNumber, cur_result );
...
{
 for( lem::Container::size_type i=0; i<links.size(); ++i )
  links[i]->GenerateConnections( points, cur_result );
