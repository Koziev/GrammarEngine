...
PatternNGrams::PatternNGrams() : explicit_scores(0)
{}
...
) const
{
    ...
    {
        res += NGramScore(ngram->Match(dict, PatternSequenceNumber, cur_result, kbase, experience));
    }

    for (auto tree_scorer : tree_scorers)
    {
        res += NGramScore(tree_scorer->Match(dict, PatternSequenceNumber, x_result, cur_result, kbase, experience, constraints, trace_log));
    }

    for (auto function : functions)
    {
        res += NGramScore(function->Match(dict, PatternSequenceNumber, x_result, cur_result, kbase, experience, constraints, trace_log));
    }
