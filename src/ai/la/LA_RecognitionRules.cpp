...
LA_RecognitionRules::~LA_RecognitionRules()
{
    for (auto rule : rules)
    {
        delete rule;
...
) const
{
    ...
    for (lem::Container::size_type i = 0; i < found_list.size(); ++i)
    {
        if (found_list[i] == wc)
        {
            ...
            if (inf == nullptr && inf_list[i] != nullptr)
                inf_matched = false;
            else if (inf != nullptr && inf_list[i] == nullptr)
                inf_matched = false;
...
) const
{
    ...
    std::unique_ptr<Solarix::LS_ResultSet> rs_words(storage->ListRecognitionRulesForWord(id_language, normalized_word));
