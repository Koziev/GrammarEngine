...
void SG_Language::LoadTxt( int Id, Macro_Parser& txt, GraphGram& gram )
{
 ...
 if( Me )
  {
   // Ёхо-сообщение: начали трансл€цию €зыка.
   ...
   while( !txt.eof() )
    {
     ...
     if( t.GetToken()==B_ALPHABET )
      ...
      {
       // —интаксис задани€ параметра: 
       // 1. скал€рный param_name = value
 ...
 if( Me )
  {
   // Ёхо-сообщение: закончили трансл€цию.
