...
{
 std::map< std::pair<int,Tree_Link>, std::pair<int,int> >::const_iterator it = raw_links2.find( std::make_pair(EntryKey0,Link) );
 
 if( it!=raw_links2.end() )
  {
   std::pair<int,int> key1 = it->second;
   return key1;
  }
...
{
 lem::Ptr<LS_ResultSet> rs( db->ListWordLinks( EntryKey0, ilink.GetState() ) );
 while( rs->Fetch() )
  {
   if( filter!=NULL )
    {
 ...
 if( outgoing )
  {
   ...
   lem::Ptr<LS_ResultSet> rs( db->ListWordLinks( EntryKey0, types ) );
 ...
 if( incoming )
  {
   ...
   lem::Ptr<LS_ResultSet> rs( db->ListWordLinks2( EntryKey0, types ) );
 ...
 if( id_phrase1!=UNKNOWN )
  {
   ...
   lem::Ptr<LS_ResultSet> rs( db->ListPhraseLinks(id_phrase1,itypes) );
   while( rs->Fetch() )
    {
                          ...
                          )
{
 ...
 if( incoming )
  {
   lem::Ptr<LS_ResultSet> rs( db->ListWordLinks2( EntryKey0, ilink.GetState() ) );
...
void SG_Net::Compile(void)
{
 ...
 lem::Ptr<Solarix::PartOfSpeechEnumerator> cenum( (PartOfSpeechEnumerator*)sg->classes().Enumerate() );
 while( cenum->Fetch() )
  { 
...
#if defined SOL_CAA
lem::Ptr<SG_ComplexLink> SG_Net::GetComplexLink( int id_link )
{
 ...
 if( db->GetPhraseLink( id_link, info ) )
  {
   ...
   return lem::Ptr<SG_ComplexLink>(lnk);
  }
 else
  {
   return lem::Ptr<SG_ComplexLink>();
  }
}
#endif
...
void SG_Net::GetComplexEntries(
                               lem::Collect< std::pair<int,lem::UFString> > &list,
                               bool lefts,
                               bool rights
                              ) const
{
 #if defined SQLITE_THESAURUS
 ...
 if( hdb!=NULL )
  {
   ...
   if( lefts && rights )
    {
     Select = "SELECT id, txt FROM TWORDS";
    }
   else if( lefts )
    {
     Select = "SELECT distinct twords.id, twords.txt FROM TWORDS, TLINKS WHERE id1=twords.id";
    }
   else if( rights )
    {
     Select = "SELECT distinct twords.id, twords.txt FROM TWORDS, TLINKS WHERE id2=twords.id";
    }
   ...
   if( res==SQLITE_OK )
     ...
     while( sqlite3_step( stmt ) == SQLITE_ROW )
      {
       ...
       UFString txt( lem::sqlite_column_ufstring(stmt,1) );
       list.push_back( std::make_pair(id,txt) );
      }
    ...
    {
     UFString msg( lem::format_str( L"SQLite error in file %s:%d message=%s", to_unicode(__FILE__).c_str(), __LINE__, lem::sqlite_errmsg(hdb).c_str() ) );
...
/*
// *************************************************************
// Собирает id сложных связок, исходящих для статьи id_phrase
// *************************************************************
void SG_Net::GetLinkedComplexEntries( int id_phrase, lem::MCollect<int> &list )
{
 lem::Ptr<LS_ResultSet> rs( db->ListPhraseLinks(id_phrase) );
 while( rs->Fetch() )
  {
...
lem::Ptr<SG_TagFilter> SG_Net::CompileTags( const lem::Collect< std::pair<UCString,UCString> > &stags ) const
{
 ...
 lem::Ptr<SG_TagFilter> ret( (SG_TagFilter*)new SG_RefTagFilter(itags,false) );
 return ret;
}
...
{
 lem::Ptr<LS_ResultSet> rs( db->ListPhraseLinks( tentry_id, link_type ) );
 while( rs->Fetch() )
  {
   const int id_link = rs->GetInt(0);
   const int id_tags = rs->GetInt(2);
...
void SG_Net::Save_SQL( OFormatter &out, const SQL_Production &sql_version, int min_entry_freq )
{
 ...
 lem::Ptr<ThesaurusTagDefsEnumerator> tags_enum(tag_defs->Enumerate());
 ...
 lem::Ptr<OFormatter> sg_link16, sg_link_tag16;
 ...
 if( sql_version.mssql_bulk_insert )
  {
   external_data = true;
   sg_link16 = new U16Formatter( path_links, true );
   sg_link_tag16 = new U16Formatter( path_link_tags, true );
  }
 else if( sql_version.loader && sql_version.IsUtf8() )
  {
   external_data=true;
   sg_link16 = new OUFormatter( path_links, false );
   sg_link_tag16 = new OUFormatter( path_link_tags, false );
  }
 else if( sql_version.loader && sql_version.IsAscii() )
  {
   external_data=true;
   sg_link16 = new OFormatter( path_links );
   sg_link_tag16 = new OFormatter( path_link_tags );
  }
 ...
 lem::Ptr<LS_ResultSet> tsenum( GetStorage().List_TagSets() );
 lem::UFString tags;
 while( tsenum->Fetch() )
  {
 ...
 lem::Ptr<WordLinkEnumerator> word_links( EnumerateWordLinks() );
 ...
 for( int sql_pass=0; sql_pass<3; sql_pass++ )
  {
   ...
   lem::Ptr<LS_ResultSet> phrase_links( db->ListPhraseLinks() );
