...
namespace
{
static bool freq_histo_sort( const std::pair<int,int> &a, const std::pair<int,int> &b )
{
 ...
 if( sql_version.top_freq!=UNKNOWN )
  {
   ...
   for( std::map<int,int>::iterator it = freq_histo.begin(); it!=freq_histo.end(); ++it )
    freqs.push_back( *it );

   // отсортируем в порядке возрастания частоты
   std::sort( freqs.begin(), freqs.end(), freq_histo_sort );

   // теперь идем по отсортированной гистограмме в порядке убывания частоты, и останавливаемся в момент, когда сумма числа статей станет > NNN
                               ...
                               )
{
 ...
 std::sort( attrs.begin(), attrs.end(), dims_sorter );
