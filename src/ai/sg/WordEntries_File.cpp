...
WordEntries_File::~WordEntries_File(void)
{
...
void WordEntries_File::CreateDefaultEntries(void)
{
 #if defined SOL_LOADTXT && defined SOL_COMPILER
...
int WordEntries_File::GetMaxMultilexemLen(void)
{
 return max_ml_len;
}
...
void WordEntries_File::PrepareForMerging(void)
{
 return;
}
...
WordEntryEnumerator* WordEntries_File::ListEntries(void)
{
...
void WordEntries_File::Delete_Entries(void)
{
 #if defined SOL_CAA
 for( lem::Container::size_type i=0; i<entry.size(); i++ )
...
CharNode* WordEntries_File::GetPrefixTreeForAllLanguages(void)
{
...
*************************************************************************/
void WordEntries_File::ProcessSynEntries(void)
{
...
void WordEntries_File::SortUp(void)
{
 if( !entry.empty() )
  {
   // Длительная операция: сообщаем пользователю.
...
void WordEntries_File::ClearGroups(void)
{
/*
...
/******************************************************************************
 ...
 учитываются).
******************************************************************************/
void WordEntries_File::BuildLookUpTable(void)
{
 if( !entry.empty() )
  {
   // Длительная операция: сообщаем пользователю.
...
LD_Seeker* WordEntries_File::Get_Seeker(void)
{
...
int WordEntries_File::size(void) const
{
 return CastSizeToInt(entry.size());
}
...
LexemeEnumerator* WordEntries_File::ListLexemes(void)
{
 return new LexemeEnumerator_File(this);
}


int WordEntries_File::CountLexemes(void)
{
...
const IntCollect& WordEntries_File::GetUEntries(void)
{
 return u_entry;
}
...
/* ???? зачем этот код здесь? загрузка в эти массивы выполняется в ::LoadBin
   if( ML_entry_key_list.find(e->GetKey())!=UNKNOWN )
    {
     for( lem::Container::size_type j=0; j<e->forms().size(); ++j )
      {
       const Lexem & form = e->forms()[j].name();
...
bool WordEntries_File::PreferPrefixSearch(void)
{
 return true;
}


int WordEntries_File::GetUnknownEntryKey(void)
{
