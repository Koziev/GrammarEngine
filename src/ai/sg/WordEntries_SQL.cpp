...
WordEntries_SQL::~WordEntries_SQL(void)
{
}


void WordEntries_SQL::CreateDefaultEntries(void)
{
}


int WordEntries_SQL::GetMaxMultilexemLen(void)
{
...
bool WordEntries_SQL::IsMultilexemHead( const lem::UCString &word, MultiLexemInfo &info )
{
 ...
 std::map<lem::UCString,Solarix::MultiLexemInfo>::const_iterator it = multilexem_head.find(word);
...
bool WordEntries_SQL::PreferPrefixSearch(void)
{
 return false;
}
...
int WordEntries_SQL::FindEntry( const lem::UCString &word_name, int id_class )
{
 ...
 std::map< std::pair<lem::UCString,int>, int >::const_iterator it = entry_class_lookup.find( std::make_pair(word_name,id_class) );
...
void WordEntries_SQL::PrepareForMerging(void)
{
 return;
}
...
CharNode* WordEntries_SQL::GetPrefixTreeForAllLanguages(void)
{
 LEM_STOPIT;
 return NULL;
}
...
LD_Seeker* WordEntries_SQL::Get_Seeker(void)
{
 LEM_STOPIT;
 return NULL;
}
...
const lem::IntCollect& WordEntries_SQL::GetUEntries(void)
{
 #if defined LEM_THREADS
...
int WordEntries_SQL::GetPredefinedEntryKey( int index )
{
 ...
 std::map< int /*index*/, int /*ekey*/ >::const_iterator it = predefined.find(index);
...
int WordEntries_SQL::FindQuantorEntry( int id_class )
{
 ...
 std::map< int /*id_class*/, int /*ekey*/ >::const_iterator it = quantors.find(id_class);
 if( it!=quantors.end() )
  return it->second;
...
const Solarix::Lexem* WordEntries_SQL::RegisterLexemeInCache( const Solarix::Lexem &lex )
{
 ...
 std::map<Solarix::Lexem,int>::const_iterator it = lexemes_map.find(lex);
...
int WordEntries_SQL::GetUnknownEntryKey(void)
{
...
SG_Entry& WordEntries_SQL::LoadEntryFromDb( int ekey )
{
 ...
 std::map<int/*ekey*/,SG_Entry*>::const_iterator it = entries_map.find(ekey);
