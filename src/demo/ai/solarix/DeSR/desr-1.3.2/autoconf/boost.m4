...
dnl If no path was given and there is a BOOST_ROOT environment variable,
dnl use it
...
#include <boost/version.hpp>
  ...
  if test "$have_boost" = "yes"; then
    WANT_BOOST_MAJOR=[`expr $boost_min_version : '\([0-9][0-9]*\)'`]
    WANT_BOOST_MINOR=[`expr $boost_min_version : '[0-9][0-9]*\.\([0-9][0-9]*\)'`]
    WANT_BOOST_SUB_MINOR=[`expr $boost_min_version : '[0-9][0-9]*\.[0-9][0-9]*\.\([0-9][0-9]*\)'`]
