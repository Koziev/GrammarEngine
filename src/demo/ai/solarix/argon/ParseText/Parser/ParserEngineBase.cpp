...
ParserEngineBase::ParserEngineBase()
{
	...
	tagger_type = -1; // -1 - none, 0 - CRF POS tagger
	parser_type = -1; // -1 - none, 0 - DeSR, 1 - top-down
	...
	for (lem::Container::size_type ijob = 0; ijob < input_ouput_filenames.size(); ++ijob)
	{
		...
		try
		{
			for (int ifile = 0; ifile < filenames.size() && sent_count < max_count; ++ifile)
			{
				...
				if (filenames.size() > 1 && print_progress)
		...
		{
			out_stream->flush();
			lem::mout->printf("Parser error: %us", e.what());
			exit(1);
		}
		catch (std::exception & e)
		{
			out_stream->flush();
			lem::mout->printf("Parser: %s", e.what());
			exit(1);
		}
		catch (...)
		{
...
void ParserEngineBase::ShowHelp()
{
